...
class HierarchicalOptimizer:
    ...
    def __init__(self, metrics_config=None, task_description: str = ""):
        ...
        self.gradient_gen = TextGradientGenerator(
            llm=self.llm, task_description=task_description
        )
        ...
        self.local_optimizer = LocalOptimizer(
            ...
            scorer=self.scorer,
            gradient_generator=self.gradient_gen,
    ...
    def optimize(
        ...
        for generation in range(1, MAX_GENERATIONS + 1):
            ...
            try:
                ...
                delta = (
                    improved_node.selection_score() - self.best_node.selection_score()
                )
                print(
                    f"  Local result: {self.best_node.selection_score():.3f} → {improved_node.selection_score():.3f} (Δ {delta:+.3f})"
                )
            ...
            print(
                f"  LLM calls this generation: {llm_calls_gen_delta} (total: {llm_calls_gen_end})"
            )
            if is_enabled():
                print(
        ...
        print(
            f"  Improvement: +{best_score - initial_score:.3f} ({((best_score - initial_score) / max(initial_score, 0.01) * 100):.1f}%)"
        )
    ...
    def get_optimization_report(self) -> Dict:
        ...
        report = {
            ...
            "results": {
                ...
                "best_score_full_validation": self.best_node.metadata.get(
                    "full_validation_score"
                ),
                "best_accuracy_full_validation": self.best_node.metadata.get(
            ...
            "best_node_lineage": [
                {
                    ...
                    "score": (
                        node.metrics.composite_score() if node.is_evaluated else None
                    ),
                }
    ...
    def compare_with_baseline(
        ...
        all_keys = sorted(
            set(
                list(baseline_metrics.metrics.keys())
                + list(optimized_metrics.metrics.keys())
            )
        )
        ...
        comparison = {
            ...
            "improvements": {
                "composite_score": optimized_metrics.composite_score()
                - baseline_metrics.composite_score(),
                **{
                    k: optimized_metrics.metrics.get(k, 0.0)
                    - baseline_metrics.metrics.get(k, 0.0)
                    for k in all_keys
                },
            },
        }
