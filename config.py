"""
Конфигурация системы оптимизации промптов.

Содержит все гиперпараметры для локальной и глобальной оптимизации,
настройки LLM-провайдера, пороговые значения метрик и конфигурации
наборов метрик для различных задач (SQuAD, генерация, GSM8K, XSUM).
"""
...
TRAIN_FAILURE_SAMPLE_SIZE: int = 40      # Сколько примеров сэмплировать из train для поиска ошибок
...
GLOBAL_CANDIDATES: int = 16              # Ширина глобального поиска
GLOBAL_HISTORY_WINDOW: int = 10          # Сколько узлов истории анализировать
GLOBAL_TEMPERATURE: float = 0.75         # Температура LLM для глобального оптимизатора
...
HISTORY_SCORE_THRESHOLD: float = 0.5     # Порог: только узлы ≥0.5 в мета-промпт
MAX_INSTRUCTION_LENGTH: int = 700        # Макс. длина инструкции
STAGNATION_SIMILARITY_THRESHOLD: float = 0.65 # Порог сходства для диагностики застоя

# ── Управление циклом оптимизации ────────────────────────────────────────────
...
FORCE_GLOBAL_AFTER_STAGNATION: int = 1   # Форсировать глобальный шаг после N поколений без улучшения
...
# ── Конфигурации метрик для различных задач ──────────────────────────────────
